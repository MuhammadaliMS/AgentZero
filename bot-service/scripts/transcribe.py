...
def load_speaker_timeline(timeline_path: str) -> list[dict]:
    """Load the speaker timeline generated by the bot's DOM tracking."""
    with open(timeline_path, "r") as f:
        data = json.load(f)
    return data.get("speakerEvents", [])
...
def run_pipeline(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if speaker_timeline_path and os.path.exists(speaker_timeline_path):
            ...
            speaker_events = load_speaker_timeline(speaker_timeline_path)
            ...
            # No speaker timeline — single speaker fallback
            print("  No speaker timeline available — all text assigned to 'Unknown'")
            final_segments = []
