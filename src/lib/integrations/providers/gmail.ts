 ...
 *   - https://www.googleapis.com/auth/gmail.send      (send/draft replies)
 *   - email                                            (identify the account)
 *   - profile                                          (display name)
 *
 * In Google Cloud Console → OAuth consent screen → Scopes, add:
 *   .../auth/gmail.readonly and .../auth/gmail.send
 * Gmail API must be enabled in APIs & Services → Library.
 */
export class GmailAuthService extends BaseAuthService {
  readonly integrationKey = 'gmail'
  ...
  async refreshToken(refreshToken: string): Promise<TokenResult> {
    ...
    return {
      access_token: data.access_token,
      refresh_token: refreshToken, // Google only issues a new refresh token on re-consent
      expires_at: new Date(Date.now() + data.expires_in * 1000).toISOString(),
      token_type: data.token_type,
    }
  }
